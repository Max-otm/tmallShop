...
<div class="row">
    <div class="col-sm-12">
        <table class="table table-bordered" style="margin: 20px">
            ...
            <tbody>
            <?php if(is_array($orderData)): foreach($orderData as $k=>$v): ?><tr oid="<?php echo ($v['oid']); ?>">
                    <td style="vertical-align: middle;text-align: center;font-size: 15px;color: orange;"  ><?php echo ($v['oid']); ?></td>
                    <td style="color: red;"><?php echo ($v['glnumber']); ?></td>
                    <td><?php echo ($v['address']); ?></td>
                    <td><?php echo ($v['remark']); ?></td>
                    <td><?php echo (date("Y-m-d H:i",$v['time'])); ?></td>
                    <td>
                        <select name="status" id="" autocomplete="off">
                            <?php if(is_array($status)): foreach($status as $key=>$vv): ?><option value="<?php echo ($vv); ?>" <?php if($v['status']==$vv): ?>selected=selected<?php endif; ?>><?php echo ($vv); ?></option><?php endforeach; endif; ?>
                        </select>
                    </td>
                    <td>
                        <a class="btn btn-danger btn-sm" href="javascript:;"onclick="del(<?php echo ($v['oid']); ?>)">删除</a>
                    </td>
        ...
        var obj = util.modal({
            ...
            events:{
                confirm:function(){
                    ...
                    location.href="<?php echo u('admin/order/delete/oid/"+oid+"');?>";
                }
            }
        });
        //显示模态框
