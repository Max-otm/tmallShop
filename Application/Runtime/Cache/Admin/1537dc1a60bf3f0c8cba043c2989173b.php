...
<table class="table table-hover" style="margin: 20px">
    ...
    <tbody>
    <?php if(is_array($cateData)): foreach($cateData as $k=>$v): ?><tr>
            <td width="80"><?php echo ($v['cid']); ?></td>
            <td><?php echo ($v['_cname']); ?></td>
            <td width="200"><input type="text" name="sort" id="" width="100" value="<?php echo ($v['csort']); ?>"></td>
            <td>
                <a class="btn btn-warning" href="<?php echo u('admin/cate/add',['cid'=>$v['cid']]);?>" style="margin-left: 20px;">添加子分类</a>
                <a class="btn btn-primary" href="<?php echo u('admin/cate/edit',['cid'=>$v['cid'],'tid'=>$v['tid']]);?>" >编辑</a>
                <a class="btn btn-danger" href="javascript:;"onclick="del(<?php echo ($v['cid']); ?>)">删除</a>
            </td>
        ...
        var obj = util.modal({
            ...
            events:{
                confirm:function(){
                    ...
                    location.href="<?php echo u('admin/cate/delete/cid/"+cid+"');?>";
                }
            }
        });
        //显示模态框
