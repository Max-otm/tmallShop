...
<ol class="breadcrumb" style="background-color: #f9f9f9;padding:8px 0;margin-bottom:10px;">
    ...
    <li class="active">
        <?php if(isset($oldData)): ?><a href="">添加子分类</a>
            <?php else: ?> <a href="">添加顶级分类</a><?php endif; ?>
    </li>
</ol>
<ul class="nav nav-tabs" role="tablist">
    <li class="active">
        <?php if(isset($oldData)): ?><a href="#tab3"><?php echo ($oldData['cname']); ?>子分类添加</a>
            <?php else: ?> <a href="">添加顶级分类</a><?php endif; ?></li>
</ul>
<form class="form-horizontal" style="width: 200px; margin-left: 25%; margin-top: 5%" method="post">
    <div class="form-group">
        <label for="Cname">分类名:</label>
        <input type="text" class="form-control" placeholder="分类" name="cname">
    </div>
    <div class="form-group">
        <label for="Cname">层次:</label>
        <select name="pid" id="Cname" class="form-control">
            <?php if(isset($oldData)): ?><option value="<?php echo ($oldData['cid']); ?>"><?php echo ($oldData['cname']); ?></option>
            <?php else: ?><option value="0">顶级分类</option><?php endif; ?>
        </select>
    </div>
    <div class="form-group">
        <label for="Tname">所属类型:</label>
        <select name="tid" id="Tname" class="form-control">
            <?php if($cid): ?><option value="0">顶级分类</option>
                <?php if(is_array($types)): foreach($types as $key=>$v): ?><option value="<?php echo ($v['tid']); ?>"><?php echo ($v['tname']); ?></option><?php endforeach; endif; ?>
                <?php else: ?><option value="0">顶级分类</option><?php endif; ?>
        </select>
    </div>
    <div class="form-group">
